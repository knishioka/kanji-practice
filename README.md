...
対象級の下にある「重点漢字を選ぶ」から、集中的に練習したい漢字を選んで「適用」します。
選択数を確認でき、「重点漢字を全解除」でその学年の選択をまとめて解除できます。

- 選択は学年ごとに端末内へ保存され、再読み込み後も維持されます。
- 未選択なら従来どおり学年全体から出題します。選択中は選んだ漢字を対象にし、除外漢字の設定を優先します。
- 同音異字・対義語などは選択内で問題を作れる組み合わせが必要です。問題がない場合は設定変更の案内を表示し、生成・印刷・PDF保存を停止します。
- プリセットや練習履歴で設定を戻しても、学年ごとの重点漢字・除外漢字は現在の選択を維持します。
...
- 印刷・PDF保存した直近の設定を端末内の練習履歴からすぐ再利用できる
